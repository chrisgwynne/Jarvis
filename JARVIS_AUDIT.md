...
| # | File | Description |
|---|---|---|
| K-1 | `audio/TtsEngine.kt` | `TextToSpeech` engine is never explicitly shut down — `shutdown()` is not called in any `destroy()`/`close()` method. Across service restarts (OEM process-kill + `START_STICKY` restart), the previous `TextToSpeech` instance may keep a binding to the TTS service alive. Fix: add a `shutdown()` method to `TtsEngine` and call it from `JarvisRuntime.stop()`. |
